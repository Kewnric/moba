...
function Segmented({ label, options, value, onChange }) {
    return (
        <div role="group" aria-label={label} className="inline-flex rounded-md border border-white/10 overflow-hidden">
            {options.map(option => (
                <button key={option.value} type="button" aria-pressed={option.value === value} onClick={() => onChange(option.value)}
                    className={`px-2.5 h-7 text-[11px] font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-400 ${option.value === value ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-400 hover:text-white'}`}>
                    {option.label}
                </button>
            ))}
        </div>
...
function PickSlot({ heroId, slot, active, team, customImages, onSelect, onClear }) {
    const filledBorder = team === 'ally' ? 'border-cyan-500/70' : 'border-red-500/70';
    return (
        <div className={`relative w-full mx-auto ${SLOT_WIDTH}`}>
            <button type="button" onClick={() => onSelect(slot)} aria-pressed={active}
                ...
                {heroId ? <HeroAvatar heroId={heroId} size="fill" className="w-full h-full" showTooltip={false} customImages={customImages} /> : <Icons.Plus size={18} />}
            </button>
            {heroId && (
...
function BanSlot({ heroId, slot, active, customImages, onSelect }) {
    return (
        <button type="button" onClick={() => onSelect(slot)} aria-pressed={active}
            aria-label={heroId ? `${slotName(slot)}: ${heroName(heroId)}` : `${slotName(slot)}: empty`}
            ...
            {heroId
                ? <HeroAvatar heroId={heroId} size="fill" className="w-full h-full grayscale" showTooltip={false} customImages={customImages} />
                : <span className="text-[10px] text-white/20">—</span>}
        </button>
    );
}

function TeamRow({ team, title, draftState, enemyLanes, customImages }) {
                ...
                {draft[banGroup].length > 0 && (
                    <div className="flex items-center gap-1">
                        ...
                        {draft[banGroup].map((heroId, index) => (
                            <BanSlot key={index} heroId={heroId} slot={{ group: banGroup, index }} active={sameSlot(activeSlot, { group: banGroup, index })} customImages={customImages} onSelect={draftState.selectSlot} />
                        ))}
                    </div>
                )}
            </div>
            <div className="grid grid-cols-5 gap-2 lg:gap-4">
                {draft[team].map((heroId, index) => (
                    <div key={index} className="min-w-0 flex flex-col items-center gap-1">
                        <PickSlot heroId={heroId} slot={{ group: team, index }} team={team} active={sameSlot(activeSlot, { group: team, index })} customImages={customImages} onSelect={draftState.selectSlot} onClear={draftState.clearSlot} />
...
export default function DraftBoard({ draftState, enemyLanes, customImages }) {
            ...
            {showSettings && (
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[11px] text-gray-400 bg-slate-900/60 border border-white/10 rounded-lg px-3 py-2">
                    ...
                    <div className="flex items-center gap-2">Bans per team
                        <Segmented label="Bans per team" value={draft.bansPerTeam} onChange={draftState.setBansPerTeam} options={BAN_CHOICES.map(count => ({ value: count, label: count === 0 ? 'None' : String(count) }))} />
                    </div>
                    <span className="text-[10px] text-gray-500">Use the numbers your draft screen shows.</span>
                </div>
            )}

            <TeamRow team="ally" title="Your team" draftState={draftState} enemyLanes={enemyLanes} customImages={customImages} />
            <TeamRow team="enemy" title="Enemy" draftState={draftState} enemyLanes={enemyLanes} customImages={customImages} />
        </section>
    );
}
